...
st.caption('Eğer ekranda tablo görünmüyorsa, aşağıdaki “Log” bölümünden hangi dosya/sheet’in neden atlandığını görebilirsiniz.')

arsiv = sorted([f for f in os.listdir(VERI_KLASORU) if f.lower().endswith(".xlsx")])

# ---- DURUM PANELİ (her zaman görünür) ----
with st.expander("🧰 Sistem Durumu (dosyalar / hızlı teşhis)", expanded=True):
    st.write(f"📁 Klasör: `{VERI_KLASORU}`")
    st.write(f"📄 Bulunan .xlsx sayısı: **{len(arsiv)}**")
    if arsiv:
...
st.info('Excel dosyaları okunuyor ve birleştiriliyor...')
