...
# Okul numaraları 9–11 hane ise bunu 9/10/11 yapmanız önerilir.
MIN_ID_LEN = 7
...
def folder_signature(folder: str) -> Tuple[Tuple[str, int], ...]:
    """Cache invalidation için: dosya adları + boyutları."""
    sig = []
    for f in list_xlsx_files(folder):
        try:
            sig.append((f, os.path.getsize(os.path.join(folder, f))))
        except Exception:
            sig.append((f, -1))
    return tuple(sig)
...
def build_result_table(folder: str, sig: Tuple[Tuple[str, int], ...]):
    """
    folder içindeki tüm .xlsx dosyaları okuyup tek tabloda birleştirir.
    sig parametresi cache invalidation içindir.
    """
    log = []
    arsiv = [f for f, _ in sig]
    if not arsiv:
        return None, log
    ...
    for f_name in ders_arsiv:
        ...
        for sh, df in df_dict.items():
            ...
            if not pc_cols:
                log.append(f"⚠️ {f_name}/{sh}: PC/PÇ kolonu bulunamadı")
                continue

            id_col = pick_id_column(df, min_len=MIN_ID_LEN)
            if not id_col:
                log.append(f"⚠️ {f_name}/{sh}: ID kolonu bulunamadı")
                continue
            ...
            all_dfs.append(temp)
            used_any = True
            log.append(f"✅ {f_name}/{sh}: eklendi | satır {after}/{before} | pc={len(pc_cols)}")

        if not used_any:
            log.append(f"ℹ️ {f_name}: hiçbir sheet kullanılmadı")
...
sig = folder_signature(VERI_KLASORU)
arsiv_names = [f for f, _ in sig]
...
if st.session_state.debug_mode:
    with st.expander("🧰 Sistem Durumu (dosyalar / hızlı teşhis)", expanded=True):
        ...
        st.write(f"📄 Bulunan .xlsx sayısı: **{len(arsiv_names)}**")
        st.write("Dosyalar:", arsiv_names)

res, log = build_result_table(VERI_KLASORU, sig)

if res is None or res.empty:
    st.error("Tablo üretilemedi. Büyük ihtimalle PC/PÇ kolonları veya ID kolonu bulunamadı.")
    if st.session_state.debug_mode:
        with st.expander("🧾 Log", expanded=True):
            st.write("\n".join(log) if log else "Log yok.")
    else:
        st.info("Sol menüden 'Teşhis Modu'nu açarsanız detaylı log görüntülenir.")
    st.stop()

# ALWAYS show a success banner so user knows processing finished
mezun_count = int((res["Durum"] == "🎓 MEZUN").sum()) if "Durum" in res.columns else 0
st.success(f"✅ Birleştirme tamamlandı: {len(res)} öğrenci | Mezun: {mezun_count} | Dosya: {len(arsiv_names)}")
